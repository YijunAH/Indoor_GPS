---
title: "Indoor Positioning System Study"
output:
...
# 8. Preparing the Test Data: online dataset
readData = 
...
# 9. Choice of Orientation (how to subset offSignalSummary?)
...
# (subsetWide$XY2<-subsetWide$XY
  subsetWide<-subsetWide%>%
  separate(XY2, c("X", "Y"), "-")
  subsetWide$X<-as.numeric(subsetWide$X)
  subsetWide$Y<-as.numeric(subsetWide$Y))
...
# 10. Finding the Nearest Neighbors
...
# return closest 2 X, Y position information

predXY = function(newSignals, newAngles, trainData, numAngles = 1, n = 3){
  ...
  estXY = lapply(closeXY, function(x) sapply(x[ , 2:3], 
                                    function(x) mean(x[1:n])))
  estXY = do.call("rbind", estXY)
  return(estXY)
}
# m: how many angles do we want to include from the offSignalSummary
# n: how many nearest neighbours do we want to average on
...
findNearestNeighbor = function(newSignal, trainSubset) {
...
# return closest 3 X, Y, X-Y position information
...
floorErrorMap(estXYk3, onlineSummary[ , c("X","Y")], 
...
v = 11
permuteLocs = sample(unique(offlineSummary$XY))
#permuteLocs[1]
#permuteLocs[100]

permuteLocs = matrix(permuteLocs, ncol = v, 
                     nrow = floor(length(permuteLocs)/v))
#Received a warning message: total 165 locations, not 166

onlineFold = subset(offlineSummary, XY %in% permuteLocs[ , 1])
...
# rewrite the reshapeSS function

offline = offline[ offline$mac != "00:0f:a3:39:dd:cd", ]
#subset the offline data

keepVars = c("XY", "X","Y", "orientation", "angle")
...
actualFold = onlineFold[ , 2:3]
calcError(estFold, actualFold)
# result: 153 (book says 186?)